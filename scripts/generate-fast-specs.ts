...
roles.forEach(role => {
  const fileName = role.toLowerCase().replace('_', '') + '.spec.ts';
  const content = `/**
 * AUTO-GENERATED: Comprehensive E2E Route Test for ${role}
 */
...
`;
  ...
  fs.writeFileSync(path.join(process.cwd(), 'tests/e2e/advanced', fileName), content);
});

console.log('✅ Spec files generated successfully.');
