...
import { redisCache, cacheKeys } from '@/lib/cache';
...
export class ApiCacheMiddleware {
  ...
  async cacheResponse(
    ...
    try {
      ...
      // Cache the response
      await redisCache.set(cacheKey, cachedResponse, finalConfig.ttl);
  ...
  async invalidateCache(pattern: string): Promise<number> {
    try {
      ...
      const keys = await redisCache.getKeys(cacheKeyPattern);
      if (keys.length > 0) {
