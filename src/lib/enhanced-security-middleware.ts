...
import { ROLES, PERMISSIONS, hasRole, hasAnyPermission, hasAllPermissions } from './auth-middleware';
  ...
  return async function (request: NextRequest, ...args: T): Promise<NextResponse> {
    ...
    try {
      ...
      if (threatDetection) {
        ...
        if (enhancedRequest.securityContext) {
          enhancedRequest.securityContext.riskScore = threatAnalysis.riskScore;
          enhancedRequest.securityContext.threatLevel = threatAnalysis.severity;
        }
        ...
        if (threatAnalysis.isBlocked) {
          await logSecurityEvent({
            ...
            severity: threatAnalysis.severity === 'critical' ? 'critical' : 'high',
            timestamp: new Date()
          });
          return secureResponse(
      ...
      if (auditLog) {
        await logSecurityEvent({
          ...
          severity: (enhancedRequest.securityContext?.riskScore || 0) > 50 ? 'medium' : 'low',
          timestamp: new Date(),
          organizationId: user?.organizationId
        });
      }
