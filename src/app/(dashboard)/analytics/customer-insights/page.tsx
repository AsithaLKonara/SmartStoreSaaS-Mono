    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white dark:text-white">Customer Insights</h1>
            <p className="text-slate-400">AI-powered customer analytics and segmentation</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="glass-dark rounded-lg shadow p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-white dark:text-white">{insights.length}</p>
            </div>
          </div>
        </div>
        ...
        <div className="glass-dark rounded-lg shadow p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-slate-400">Avg Growth Rate</p>
              <p className="text-2xl font-bold text-white dark:text-white">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {segments.map((segment) => (
                    <div key={segment.segment} className="p-4 border border-white/5 rounded-lg">
                      <div className="flex items-center justify-between mb-2">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${getSegmentColor(segment.segment)}`}>
                          {segment.segment}
                        </span>
                        <span className="text-sm text-slate-400">{segment.count} customers</span>
                      </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {insights
                    ...
                    .slice(0, 10)
                    .map((insight, index) => (
                      <div key={insight.id} className="flex items-center justify-between p-3 border border-white/5 rounded-lg">
                        <div className="flex items-center">
                          <div className="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center text-sm font-medium text-slate-400 mr-3">
                            {index + 1}
                          </div>
                          <div>
                            <div className="font-medium">{insight.name}</div>
                            <div className="text-sm text-slate-400">{insight.email}</div>
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="font-semibold">{formatCurrency(insight.totalSpent)}</div>
                          <div className="text-sm text-slate-400">{insight.totalOrders} orders</div>
                        </div>
                      </div>
                    ))}
                </div>
          ...
          {activeTab === 'segments' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {segments.map((segment) => (
                  <div key={segment.segment} className="p-6 border border-white/5 rounded-lg">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                {filteredInsights.map((insight) => (
                  <div key={insight.id} className="p-6 border border-white/5 rounded-lg">
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <h4 className="text-lg font-semibold">{insight.name}</h4>
                        <p className="text-slate-400">{insight.email}</p>
                      </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-white/5">
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                          <div className="text-sm text-slate-400">Avg Days Between Orders</div>
                    ...
                    {insight.preferredCategories.length > 0 && (
                      <div className="mt-4 pt-4 border-t border-white/5">
                        <div className="text-sm text-slate-400 mb-2">Preferred Categories</div>
